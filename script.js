...
document.addEventListener('DOMContentLoaded', function() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    // Verificar que los números no hayan sido tomados mientras se llenaba el formulario
    Promise.all(selectedNumbers.map(num => db.collection('rifa').doc(num).get()))
      .then(snapshots => {
        ...
        if (yaOcupados.length > 0) {
          ...
          // Registrar en la colección compradores
          db.collection('compradores').add({
            ...
            selectedNumbers.forEach(num => {
              db.collection('rifa').doc(num).set({ reservado: true });
